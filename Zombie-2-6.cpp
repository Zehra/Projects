...
static void setVar(int playerID, const std::string& key, const std::string& value)
{
...
static void setVar(int playerID, const std::string& key, const float value)
{
    char buf[64];
    snprintf(buf, sizeof(buf), "%f", value);
    setVar(playerID, key, buf);
}
...
void setZombie(int playerID)
{
    ...
    bz_changeTeam(playerID, eGreenTeam);
}
...
void setHuman(int playerID)
{
    ...
    bz_changeTeam(playerID, eRedTeam);
}
...
{
public:
    virtual const char* Name() { return "Zombie"; }
...
void Zombie::Init(const char*config) {
